...
### 同樣規則適用於 AI 潤稿模型（POLISH_MODELS）

`POLISH_MODELS` 的維護規則與 `STT_MODELS` **完全相同**：

1. **`src/core/stt/models.ts`** — `POLISH_MODELS` 的單一常數來源；各 provider（groq/openai/gemini/claude）的精選模型陣列與 `DEFAULT_POLISH_MODEL` 定義於此。
2. **`app-tauri/ui/settings.js`** — `POLISH_MODELS` 的刻意鏡像副本；webview 沒有 bundler，無法 `import`，因此手動複製一份。

> **更新潤稿模型清單時，兩處必須同步修改，缺一不可。**
> 若不一致，Settings「AI 潤稿」分頁的模型下拉與 sidecar 實際使用的模型可能不符。

Settings 末項「自訂…」逃生口同樣適用，可輸入任意 model string 覆蓋精選清單落後的情況。**無 runtime model-list fetch**，理由同 STT。
